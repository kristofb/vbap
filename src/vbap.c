...
// read in loudspeaker matrices
{
 	...
 	if(ac>0) 
	{
		...
		if (d!=2 && d!=3) { object_error(&x->x_ob,"vbap %s: Dimension can be only 2 or 3",s->s_name); x->x_lsset_available=0; return; }

		x->x_dimension = d;
		x->x_lsset_available=1;
	}
 	else { object_error(&x->x_ob,"vbap %s: bad empty parameter list",s->s_name); x->x_lsset_available=0; return; }
	...
	if(ac>1) 
	{
		...
		else { object_error(&x->x_ob,"vbap: ls_amount NaN");  x->x_lsset_available=0; return; }
	...
	long counter = (ac - 2) / ((x->x_dimension * x->x_dimension*2) + x->x_dimension);
 	...
 	if(counter==0) { object_error(&x->x_ob,"vbap %s: not enough parameters",s->s_name); x->x_lsset_available=0; return; }
 	...
 	while(counter-- > 0)
	{
 		for(i=0; i < x->x_dimension; i++)
		{
 			if(av[datapointer].a_type == A_LONG)
			{
 			...
 			else { object_error(&x->x_ob,"vbap %s: param %d is not an int",s->s_name,datapointer); x->x_lsset_available=0; return; }
 		}	
 		for(i=0; i < x->x_dimension*x->x_dimension; i++)
		{
 			if(av[datapointer].a_type == A_FLOAT)
			{
 			...
 			else { object_error(&x->x_ob,"vbap %s: param %d is not a float",s->s_name,datapointer); x->x_lsset_available=0; return; }
 		}
