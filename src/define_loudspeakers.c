...
// convert angular direction to cartesian
{
  float azi = ls->azi;
  float ele = ls->ele;
  ls->x = cosf((float) azi * atorad) * cosf((float) ele * atorad);
  ls->y = sinf((float) azi * atorad) * cosf((float) ele * atorad);
  ls->z = sinf((float) ele * atorad);
}
...
void initContent_ls_directions(t_def_ls *x,int ac,t_atom*av)
{
	...
	else { object_error(&x->x_ob,"define-loudspeakers: dimension NaN"); return; }
	...
	for(int i=0; i < x->x_def_ls_amount;i++)
	{
		...
		else { object_error(&x->x_ob,"define-loudspeakers: direction angle #%d NaN",i+1); x->x_ls_read = 0; return; }
		...
		{  // 3-D 
			...
			else { object_error(&x->x_ob,"define-loudspeakers: elevation #%d NaN",i+1);  x->x_ls_read = 0; return; }
